...
\lambda_{i}^{*}c_{i}(x^{*}) & = & 0,\quad\forall i\in\mathcal{E}\cup\mathcal{I}\end{eqnarray*}

:::

Next, we introduce the notion of _strong duality_ which is fundamental in the theory of linear programming. 
...
In practice, we do not need to start from the general expression in  @eq-dual2 to derive the dual problem. Instead, we can simply follow the steps introduced above: write the primal problem in standard form and then mechanically translate it into its dual counterpart.