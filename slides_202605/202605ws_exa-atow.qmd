...
- [optimal scheduling for planning]{.bg style="--col: #F5C946"}
...
## SLP Metrics {.smaller}
### When is a stochastic solution needed?

There are 3 possible scenarios when uncertainty is present:

1. Expected Value of the Mean, $\text{EVM},$ where mean values are considered.
2. Expected Value of the Stochastic Solution $\text{EVSS},$ by solving the recourse problem (RP).
3. Expected Value with Perfect Information $\text{EVPI},$  where an oracle has provided full second-stage (future) information.

These are related by the inequality,
$$ \text{EVM} \le \text{EVSS} \le \text{EVPI}.
$$

This inequality yields two stochastic performance [metrics]{.ubdteal}:

1. Value of the [stochastic solution]{.ubdyellow} $\text{VSS} =  \text{EVSS} - \text{EVM}.$
2. Value of [perfect information]{.ubdyellow} $\text{VPI} =  \text{EVPI} - \text{EVSS}.$

::: callout-important
[**VSS**]{.ubdred} measures the possible gain, or value, from considering uncertainty, and is the consequence of a hedging strategy.
:::
...
We are preparing a complete toolbox for shadowing the Digital Continuum. Optimization is fully documented at 
...
## Optimal Scheduling Online User-Guide

- general, accessible intro
- theory: deterministic LP, stochastic LP
- many, many examples (with CCDS context)
- use-cases (under construction)

::: footer
Learn more: [https://markasch.github.io/RCP4CDT/](https://markasch.github.io/RCP4CDT/)
:::
