...
The chapter is divied into deterministic (D0 to D3) and stochastic (S1 to S3) scenarios.  Realistic cases are presented in the Use-Cases chapter.



## D1. Simplified Supply Chain Example
...
## D2. Simplified Scheduling Example

- empirical scheduling
- multiple machine, disjunctive scheduling
...
Though, for <mark>interoperability</mark>, we should privilege the data model format, described above in **CDT-Implementation**. 
...
As presented in the [Risk Measures](./th-stochastic.qmd#risk-measures) section, there are various ways to include risk in the cost function. In this example, we compare...
...
To only have code displayed, but not executed use `{.python}`...
